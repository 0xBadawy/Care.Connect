...
namespace Server
{
    public class DistanceService
    {
        ...
        public Dictionary<string, double> CalculateDistanceAPI(string location)
        {
            ...
            Tuple<double, double> myLocation = Tuple.Create(x, y);
            ...
            {
                string distanceStr = item.Item2.ToString("F2"); // Format to two decimal places
                string durationStr = item.Item3.ToString("F2"); // Format to two decimal places

                result += $"Hospital {item.Item1} : {distanceStr} KM - Time {durationStr} Minutes\n";
